...
pub fn LearnedClauseStats(comptime Proof: type) type {
    return struct {
...
pub fn ClauseStats(comptime Proof: type) type {
    return union(enum) {
        Learned: LearnedClauseStats(Proof),
        Initial,
    };
}

/// clause description
pub fn Clause(comptime Proof: type) type {
    return struct {
        /// statistics of the clause
        stats: ClauseStats(Proof),
...
pub fn ClauseManager(comptime Proof: type) type {
    return struct {
        pub const ClauseRef = *Clause(Proof);
        ...
        pub fn initClause(self: *Self, is_learned: bool, expr: []const Lit, proof: Proof) !ClauseRef {
            var new_expr: []Lit = undefined;
            var clause: ClauseRef = undefined;

            if (is_learned) {
                new_expr = try self.main_allocator.allocator().alloc(Lit, expr.len);
                clause = try self.main_allocator.allocator().create(Clause(Proof));

                try self.learned_clauses.append(clause);

                var st = LearnedClauseStats(Proof){ .cref = clause };
                clause.stats = ClauseStats(Proof){ .Learned = st };
            } else {
                new_expr = try self.allocator.alloc(Lit, expr.len);
                clause = try self.allocator.create(Clause(Proof));

                try self.initial_clauses.append(clause);

                clause.stats = ClauseStats(Proof).Initial;
            }
        ...
        pub fn garbadgeCollect(self: *Self, fraction: f64) !void {
            ...
            while (i < self.learned_clauses.items.len) {
                ...
                if (cref.lock == 0 and (delete or cref.is_deleted())) {
                    ...
                    var new_cref = try arena.create(Clause(Proof));
...
test "random clause manager test" {
    ...
    inline while (step < 10) : (step += 1) {
        var cm = ClauseManager(void).init(allocator);
        defer cm.deinit();
